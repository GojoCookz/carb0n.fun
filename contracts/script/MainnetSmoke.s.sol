...
contract MainnetSmoke is Script {
    ...
    function run() external {
        require(block.chainid == 1, "MainnetSmoke: Ethereum mainnet only");

        uint256 pk = vm.envUint("PRIVATE_KEY");
        address me = vm.addr(pk);
        require(me != COMPROMISED_DEPLOYER, "MainnetSmoke: that key's private key is PUBLIC");
        ...
        require(LAUNCHER.code.length > 0, "MainnetSmoke: no code at LAUNCHER");
        require(FEE_HOOK.code.length > 0, "MainnetSmoke: no code at FEE_HOOK");
        require(PAIR.code.length > 0, "MainnetSmoke: no code at SMOKE_PAIR");
        require(pairSeed > 0, "MainnetSmoke: set SMOKE_PAIR_SEED (in pair units) - there is no safe default");

        Launcher launcher = Launcher(LAUNCHER);
        FeeHook hook = FeeHook(FEE_HOOK);
        IERC20 pair = IERC20(PAIR);
        IPoolManager mgr = IPoolManager(Addresses.MAINNET_POOL_MANAGER);

        uint256 haveBefore = pair.balanceOf(me);
        require(haveBefore >= pairSeed * 3, "MainnetSmoke: hold at least 3x SMOKE_PAIR_SEED - buy, sell and slippage");
        ...
        console.log("");
        console.log("=== SMOKE PASSED on Ethereum mainnet ===");
